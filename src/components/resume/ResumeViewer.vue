...
onMounted(() => {
  const jsonResumeDataString = JSON.stringify(props.resume_data, null, 2); 
  console.log('Resume_Data:', jsonResumeDataString);
  
  const jsonMetadataString = JSON.stringify(props.metadata, null, 2);
  console.log('Metadata:', jsonMetadataString);
  
  const jsonHeader_dataString = JSON.stringify(props.header_data, null, 2); 
  console.log('Header_data:', jsonHeader_dataString);

  const jsonTemplateString = JSON.stringify(props.template, null, 2); 
  console.log('Template:', jsonTemplateString);
});
